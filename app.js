(function(){
  ...
  const els = {
    ...
    voice: $('#voiceSelect'), wordCount: $('#wordCount'),
    progress: $('#progress'), now: $('#nowReading'), estimates: $('#estimates'), langHint: $('#langHint'),
  ...
  function save(){ try{ localStorage.setItem('tts_text', els.text?.value||''); localStorage.setItem('tts_voiceURI', els.voice?.value||''); localStorage.setItem('tts_rate', els.rate?.value||'1'); localStorage.setItem('tts_pitch', els.pitch?.value||'1'); localStorage.setItem('tts_volume', els.volume?.value||'1'); localStorage.setItem('tts_cast', JSON.stringify(state.cast)); }catch{} }
  function load(){ try{ const t=localStorage.getItem('tts_text'); if(t&&els.text) els.text.value=t; const r=localStorage.getItem('tts_rate'); if(r&&els.rate) els.rate.value=r; const p=localStorage.getItem('tts_pitch'); if(p&&els.pitch) els.pitch.value=p; const v=localStorage.getItem('tts_volume'); if(v&&els.volume) els.volume.value=v; const cast = localStorage.getItem('tts_cast'); if(cast) state.cast=JSON.parse(cast); updateSliderLabels(); updateCounts(); }catch{} }
  ...
  function estimate(){ const words=(els.text?.value.trim().match(/\S+/g)||[]).length; const rateFactor=parseFloat(els.rate?.value||'1'); const wpm=state.wpmBase*rateFactor; const mins=words/Math.max(1,wpm); els.estimates && (els.estimates.textContent = words?`≈ ${formatTime(mins)} • ${words.toLocaleString()} palabras`:'–'); }
  ...
  function updateSliderLabels(){ els.rateVal&&(els.rateVal.textContent=Number(els.rate.value).toFixed(1)); els.pitchVal&&(els.pitchVal.textContent=Number(els.pitch.value).toFixed(1)); els.volumeVal&&(els.volumeVal.textContent=Number(els.volume.value).toFixed(2)); }
  ...
  function populateVoices(){
    ...
    if(els.voice){
      els.voice.innerHTML='';
      state.voices.forEach(v=>{ const opt=document.createElement('option'); opt.value=v.voiceURI; opt.textContent=`${v.name} — ${v.lang}${v.default?' (predeterminada)':''}`; els.voice.appendChild(opt); });
      const saved=localStorage.getItem('tts_voiceURI'); let idx=state.voices.findIndex(v=>v.voiceURI===saved);
      ...
      els.langHint && (els.langHint.textContent=v?(v.lang||'—'):'—');
    }
  ...
  function renderCast(){
    ...
    state.cast.forEach((c,i)=>{
      ...
      const btnPara=document.createElement('button'); btnPara.textContent='🧩 Párrafo'; btnPara.className='ghost'; btnPara.onclick=()=>{ if(!c.name){ els.live&&(els.live.textContent='Pon nombre al personaje antes de asignar.'); return; } wrapParagraphWithTag(c.name); };
      const btnTest=document.createElement('button'); btnTest.textContent='🔊 Prueba'; btnTest.onclick=()=> previewCharacter(c);
      const btnDel=document.createElement('button'); btnDel.textContent='🗑️'; btnDel.className='danger'; btnDel.onclick=()=>{ state.cast=state.cast.filter(x=>x.id!==c.id); els.live&&(els.live.textContent=`Eliminado ${c.name||'personaje'}.`); save(); renderCast(); renderQuickCast(); renderTagBar(); };
  ...
  function renderQuickCast(){
    ...
    state.cast.forEach((c,i)=>{
      ...
      const bPar = document.createElement('button'); bPar.textContent='🧩 Párrafo'; bPar.title='Asignar a párrafo'; bPar.disabled = !c.name; bPar.onclick = ()=>{ if(!c.name){ els.live&&(els.live.textContent='Pon nombre al personaje en el panel inferior.'); return; } wrapParagraphWithTag(c.name); };
  ...
  function ensureCastForUsedSpeakers(usedNames){
    const narratorURI=getSelectedVoice()?.voiceURI; const sameLang=state.voices.filter(v=>v.lang===getSelectedVoice()?.lang)||state.voices; let rr=parseInt(localStorage.getItem('tts_rr_idx')||'0',10);
    const have=new Set(state.cast.map(c=>norm(c.name)));
    usedNames.forEach(nm=>{ if(have.has(norm(nm))) return; const nc={ id:uniqueId(), name:nm, voiceURI:narratorURI, rate:1, pitch:1, volume:1, color: COLORS[state.cast.length%COLORS.length]}; const candidates=sameLang.filter(v=>v.voiceURI!==narratorURI); if(candidates.length){ const pick=candidates[rr % candidates.length]; rr++; nc.voiceURI=pick.voiceURI; } state.cast.push(nc); });
    localStorage.setItem('tts_rr_idx', String(rr)); save(); renderCast(); renderQuickCast(); renderTagBar();
  }
  ...
  function start(){
    ...
    const missing=usedNames.filter(nm=>!currentSettingsForSpeaker(nm)); if(missing.length&&els.live){ els.live.textContent=`Sin voz asignada: ${missing.join(', ')}. Uso voz por defecto.`; }
    speakNext();
  }
  function speakNext(){
    ...
    const u=new SpeechSynthesisUtterance(item.text); const defV=getSelectedVoice(); const settings=currentSettingsForSpeaker(item.speaker); const v=settings?.voice||defV; if(v) u.voice=v;
    u.lang=v?.lang || (defV?.lang) || 'es-ES'; u.rate=settings?.rate ?? (parseFloat(els.rate?.value||'1')); u.pitch=settings?.pitch ?? (parseFloat(els.pitch?.value||'1')); u.volume=settings?.volume ?? (parseFloat(els.volume?.value||'1'));
  ...
  function renderTagBar(){ const bar=els.tagBar; if(!bar) return; bar.innerHTML=''; const items=[{name:'Narrador', color:'#64748b'}, ...state.cast.map((c,i)=>({name:c.name||`P${i+1}`, color:c.color||COLORS[i%COLORS.length]}))]; items.forEach(it=>{ const b=document.createElement('button'); b.className='chip'; b.draggable=true; b.title=`Arrastra para etiquetar como ${it.name}`; const d=document.createElement('span'); d.className='dot'; d.style.background=it.color; const t=document.createElement('span'); t.textContent=it.name; b.append(d,t); b.addEventListener('dragstart',e=>{ e.dataTransfer.setData('text/x-speaker', it.name); e.dataTransfer.effectAllowed='copyMove'; }); bar.appendChild(b); }); buildTagOptions(); }
  ...
  function installResizer(){
    ...
    res.addEventListener('keydown',e=>{ const step=(e.shiftKey?32:16); const rect=grid.getBoundingClientRect(); const cur=parseInt(localStorage.getItem('grid_left_px')||'0',10) || Math.round(rect.width*0.64); if(e.key==='ArrowLeft'){ applyCols(cur-step); e.preventDefault(); } if(e.key==='ArrowRight'){ applyCols(cur+step); e.preventDefault(); } if(e.key==='Home'){ applyCols(480); e.preventDefault(); } if(e.key==='End'){ applyCols(rect.width - 280 - 10 - 18); e.preventDefault(); } });
