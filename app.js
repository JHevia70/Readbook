// Readbook / app.js v1.3.4 — Narrador fijo + Añadir personaje + lector funcional
(function(){
  ...
  function init(){
    ...
    if(els.btnAuto) els.btnAuto.addEventListener('click', ()=>{
      const names=autodetectNames(els.text?.value||''); names.forEach(n=>{ if(!state.cast.some(c=>norm(c.name)===norm(n))) addCharacter(n); });
      renderTagBar();
    });
    ...
    els.selfCheck?.addEventListener('click', ()=>{
      const issues=[];
      if(!els.text) issues.push('No textarea');
      if(!els.castList) issues.push('No castList');
      if(!els.play) issues.push('No botón Leer');
  ...
  function autodetectNames(t){
    const names=new Set();
    // Líneas tipo "NOMBRE: diálogo"
    const re1=/^\s*([A-Za-zÁÉÍÓÚÜÑáéíóúüñ0-9'\.\- ]{1,40})\s*:/gm;
    let m; while((m=re1.exec(t))) names.add(m[1].trim());
    // Bloques [[NOMBRE]]...[[/NOMBRE]]
    const re2=/\[\[([^\]]+)\]\]/g; while((m=re2.exec(t))) names.add(m[1].trim());
    // limpia falsos
