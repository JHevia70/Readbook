(function(){
  ...
  // Minimal theme adapter (no-op if your app doesn't use themes)
  ...
  // Storage
  function save(){ try{
    if(els.text) localStorage.setItem('tts_text', els.text.value);
    if(els.voice) localStorage.setItem('tts_voiceURI', els.voice.value || '');
    if(els.rate) localStorage.setItem('tts_rate', els.rate.value);
    if(els.pitch) localStorage.setItem('tts_pitch', els.pitch.value);
    if(els.volume) localStorage.setItem('tts_volume', els.volume.value);
  ...
  function renderCast(){
    ...
    state.cast.forEach((c, i)=>{
      ...
      btnApply.onclick = ()=>{ const hasSel = (els.text.selectionEnd - els.text.selectionStart) > 0; if(!c.name){ if(els.live) els.live.textContent='Pon nombre al personaje antes de asignar.'; return; } if(!hasSel){ if(els.live) els.live.textContent='Selecciona un trozo de texto en el editor.'; return; } wrapSelectionWithTag(c.name); };
      const btnPara = document.createElement('button'); btnPara.textContent='🧩 Párrafo'; btnPara.title='Aplicar este personaje al párrafo actual'; btnPara.className='ghost'; btnPara.onclick = ()=>{ if(!c.name){ if(els.live) els.live.textContent='Pon nombre al personaje antes de asignar.'; return; } wrapParagraphWithTag(c.name); };
      const btnTest = document.createElement('button'); btnTest.textContent='🔊 Prueba'; btnTest.onclick = ()=> previewCharacter(c);
      const btnDel = document.createElement('button'); btnDel.textContent='🗑️'; btnDel.className='danger'; btnDel.title='Eliminar'; btnDel.onclick = ()=>{ state.cast = state.cast.filter(x=>x.id!==c.id); if(els.live) els.live.textContent=`Eliminado ${c.name||'personaje'}.`; save(); renderCast(); };
  ...
  function parseScript(t){
    ...
    // 1) Bloques [[NAME]] ... [[/NAME]]  — usa [\s\S] para abarcar cualquier char
    const out = [];
    const re = /\[\[([^\]]+)\]\]([\s\S]*?)\[\[\/\1\]\]/g;
    let last = 0; let m;
    while((m = re.exec(t))){
  ...
  function start(){
    ...
    if(!textRaw){ if(els.live) els.live.textContent = 'No hay texto para leer.'; return; }
  ...
  function speakNext(){
    ...
    u.onerror = (e) => { console.error('TTS error', e); if(els.live) els.live.textContent = 'Error al sintetizar voz.'; finish(); };

    window.speechSynthesis.speak(u);
  }

  function pause(){ if(window.speechSynthesis.speaking && !window.speechSynthesis.paused){ window.speechSynthesis.pause(); state.paused = true; if(els.live) els.live.textContent = 'Pausado.'; } }
  function resume(){ if(window.speechSynthesis.paused){ window.speechSynthesis.resume(); state.paused = false; if(els.live) els.live.textContent = 'Reanudando…'; } else if(!state.speaking){ start(); } }
  function stop(){ state.canceling = true; window.speechSynthesis.cancel(); finish(); }
  function finish(){ state.speaking = false; state.paused = false; state.idx = 0; state.queue = []; if(els.now) els.now.textContent = 'Listo.'; if(els.progress) els.progress.value = 0; disableEditing(false); }
  ...
  function autoDetect(){
    ...
    if(els.live){ els.live.textContent = added===0 ? 'No se encontraron nombres nuevos.' : `Añadidos ${added} personaje(s).`; }
  }

  function readFile(file){ if(!file) return; const reader = new FileReader(); reader.onload = () => { if(els.text){ els.text.value = normalizeText(String(reader.result||'')); save(); updateCounts(); } }; reader.onerror = () => { if(els.live) els.live.textContent = 'No se pudo leer el archivo.'; }; reader.readAsText(file); }
  ...
  function runSelfCheck(){
    ...
    if(!toolbar){ issues.push('No existe toolbar bajo el editor'); }
      ...
      btns.forEach(b=>{ const txt = (b.textContent||'').trim(); if(txt.includes('<') || txt.includes('>')) issues.push(`Botón con texto sospechoso: ${b.id||txt.slice(0,20)+'…'}`); });
    }
  ...
  // Quick tagging (UI chips -> DnD into textarea)
  ...
  function wrapParagraphWithTag(name){ const el = els.text; if(!el) return; el.focus(); const start = el.selectionStart ?? 0; const end   = el.selectionEnd ?? start; const {start:s, end:e} = getParagraphBounds(el.value, start, end); const inner = el.value.slice(s, e); if(!inner.trim()){ if(els.live) els.live.textContent = 'Párrafo vacío o solo espacios.'; return; } if(inner.includes('[[') && inner.includes(']]')){ if(els.live) els.live.textContent = 'Ese párrafo ya tiene etiquetas. Usa "Asignar sel." o limpia antes.'; return; } const before = el.value.slice(0, s); const after  = el.value.slice(e); const open = `[[${name}]] `, close = ` [[/${name}]]`; el.value = before + open + inner + close + after; const caret = (before + open).length; el.setSelectionRange(caret, caret); save(); updateCounts(); }
  ...
  // --- Column resizer (drag or arrow keys) ---
  function installResizer(){
    const grid = document.querySelector('main .grid');
    if(!grid) return;

    // If already installed, skip
    if(grid.querySelector('.col-resizer')) return;

    // Create resizer div and insert as middle column
    ...
    // Place between first and second section
    if(grid.children.length >= 2){
      grid.insertBefore(res, grid.children[1].nextSibling);
    } else {
    ...
    // Restore saved width or default ratio
    const saved = parseInt(localStorage.getItem('grid_left_px')||'0', 10);
    if(saved){
      applyCols(saved);
    } else {
      const rect = grid.getBoundingClientRect();
      const initial = Math.max(560, Math.round(rect.width * 0.64)); // 64% to editor
      applyCols(initial);
    }
    ...
    // Keyboard: arrows adjust 16px; Home/End snap
  ...
  if(els.preview) els.preview.addEventListener('click', preview);
  if(els.exportBtn) els.exportBtn.addEventListener('click', exportTxt);
  if(els.resetBtn) els.resetBtn.addEventListener('click', ()=>{ if(els.text){ els.text.value=''; save(); updateCounts(); } });
  ...
  if(els.voice) els.voice.addEventListener('change', ()=>{ save(); const v = getSelectedVoice(); if(els.langHint) els.langHint.textContent = v? (v.lang||'—') : '—'; });
